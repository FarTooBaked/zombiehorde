...
{
    [Info("ZombieHorde", "k1lly0u", "0.6.9")]
    class ZombieHorde : RustPlugin, IChaosNPCPlugin
    {
        ...
        private static bool IsInOrOnBuilding(BaseEntity baseEntity)
        {
            ...
            if (Physics.Raycast(position, Vector3.up, out raycastHit, 40f, CONSTRUCTION_LAYER) || 
        ...
        private Vector3 GetSpawnPoint()
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                if (NavmeshSpawnPoint.Find(position, 25f, out position))
                {
                    ...
                    if (WaterLevel.GetWaterDepth(position, true, true) <= 0.01f)
                        return position;
                }
            }
        ...
        public class Horde
        {
            ...
            private void SetSleeping(bool sleep)
            {
                ...
                for (int i = 0; i < members.Count; i++)
                {
                    ...
                    (zombieNPC.Brain as ZombieNPCBrain).SetSleeping(sleep);
                }
        ...
        public class ZombieNPC : CustomScientistNPC, IAIAttack, IAISenses, IThinker
        {
            ...
            {
                if (!IsAlive() || IsDestroyed || noiseEmitter == null || noiseEmitter.IsDestroyed)
                    return;
            ...
            private void LiquidWeaponFireTick()
            {
                ...
                if (Physics.Raycast(ray, out raycastHit, currentRange, 1218652417))
                {
            ...
            private void AttemptMount(BasePlayer player, BaseVehicle baseVehicle)
            {
                ...
                if (GetIdealMountPoint(baseVehicle, player, out idealMountPoint))
                {
        ...
        public class ZombieNPCBrain : CustomScientistBrain
        {
            ...
            public class ZombieAttackState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    BaseEntity baseEntity = brain.Events.Memory.Entity.Get(brain.Events.CurrentInputMemorySlot);
                    if (baseEntity == null || !zombieNpc.CanTargetEntity(baseEntity) || (baseEntity is BasePlayer && !zombieNpc.CanTargetBasePlayer(baseEntity as BasePlayer)))
                    {
                        brain.Navigator.ClearFacingDirectionOverride();
                        attack.StopAttacking();
                        return StateStatus.Finished;
            ...
            public class ZombieChaseState : BasicAIState
            {
                ...
                public override StateStatus StateThink(float delta, BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    if ((attackEntity is BaseMelee || attackEntity is BaseProjectile) && baseEntity is BasePlayer)
                    {
                        ...
                        {
                            if (brain.Navigator.IsSwimming() && baseEntity is BasePlayer && !(baseEntity as BasePlayer).isMounted)
                                goto ContinueEvent;
            ...
            public class ZombieMountedState : BasicAIState
            {
                ...
                public override void StateLeave(BaseAIBrain brain, BaseEntity entity)
                {
                    ...
                    if ((entity as ZombieNPC).isMounted)
                        (entity as ZombieNPC).TryDismount();
                }
